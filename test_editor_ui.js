...
async function configKeysCoverage() {
  ...
  check(schemaKeys.length === 148, "配置 schema 里是 148 个键", schemaKeys.length);
  ...
  check(cov.counts.total === 148 &&
    cov.counts.numbers + cov.counts.tab + cov.counts.panel === 148,
    "148 个键全都有归属（数值页 / 别的页 / 插件面板）", JSON.stringify(cov.counts));
  ...
  check(numKeys.length === 148 && schemaKeys.every(function (k) { return numKeys.indexOf(k) >= 0; }),
    "数值页按全量清单铺开 148 行（一行都没少）", numKeys.length + " 行");
  ...
  check(T.keysFilteredRows().length === 1 &&
    ...
    "按配置键精确搜索只留那一行");
  T.state.keysQuery = "";
  check(T.keysFilteredRows().length === 148, "清空搜索词 -> 又看到全部 148 个键");
  ...
  check(demo.length === 148 && demo.filter(function (r) { return !r.group; }).length === 0,
    "演示数据 148 行且每行都有分组（数值页的分组标题撑得起来）",
    demo.length + " 行");

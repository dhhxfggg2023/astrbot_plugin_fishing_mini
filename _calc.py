...
def _parse_bait_defs(raw: Any, *, warn: Any = None) -> dict[str, dict[str, Any]]:
    """解析鱼饵定义。返回 {bait_id: {...}}，一定包含 none（空钩）。

    新格式（10 段）：
        ``id|名称|emoji|单价|一组数量|品质幸运|稀有度权重|解锁等级|需要鱼竿|说明``
    旧格式（8~9 段）：没有解锁等级/需要鱼竿 —— 一律视为 1 级、无鱼竿要求
    （向后兼容：老配置不会因为格式不同把鱼饵锁死）。

    坏行会被跳过；传了 ``warn`` 时把「跳了几行」说出来（同 `_parse_item_defs`）。
    """
    ...
    for entry in items:
        if not isinstance(entry, str):
            skipped.append(str(entry)[:40])
            continue
        parts = [p.strip() for p in entry.split("|")]
        if len(parts) < 7:
            skipped.append(entry[:40])
            continue
    ...
    baits["none"]["need_rod"] = ""
    if warn and (skipped or duplicate):
        bits: list[str] = []
        if skipped:
            bits.append(f"{len(skipped)} 行格式不对已跳过（首条：{skipped[0]}）")
        if duplicate:
            bits.append(f"{len(duplicate)} 个 id 重复（{'、'.join(duplicate[:3])}）")
        warn("；".join(bits))
    return baits
...
def _unknown_effect_keys(text: Any) -> list[str]:
    """效果串里**插件不认识的键**（没写 ``=``、或不在白名单里的），供解析器告警。

    与 `_parse_effects` 共用同一套判定（`_effect_allowed` + 别名）：``quality_up``
    这类旧写法算认识，只有真正会被**静默丢掉**的那几种才列出来。效果键写错不会让
    道具消失，但会让它「形同虚设」—— 所以要把「你以为配了、其实没生效」喊出来。
    """
    allowed = _effect_allowed()
    aliases = EFFECT_ALIASES_EXT
    bad: list[str] = []
...
def _parse_item_defs(raw: Any, *, warn: Any = None) -> dict[str, dict[str, Any]]:
    """解析道具定义。返回 {item_id: {...}}。
    ...
    **解锁等级**（省略 = 1 级，老配置照常读）；
    ...
    for entry in entries:
        ...
        if not item_id or not name:
            ...
            continue
        if item_id in items:
            duplicate.append(item_id)          # 后一条覆盖前一条，说一声
        for token in _unknown_effect_keys(parts[5]):
            unknown_fx.append(f"{item_id} 的 {token}")
        items[item_id] = {
            "id": item_id,
            "name": name,
            "emoji": emoji,
    ...
    if warn and (skipped or duplicate or unknown_fx):
        ...
        if duplicate:
            bits.append(
                f"{len(duplicate)} 个 id 重复（后一条覆盖前一条："
                f"{'、'.join(duplicate[:3])}）"
            )
        if unknown_fx:
            bits.append(f"{len(unknown_fx)} 处效果键不认识（{'；'.join(unknown_fx[:3])}）")
        warn("；".join(bits))
    return items
...
def _parse_rod_defs(raw: Any, *, warn: Any = None) -> list[dict[str, Any]]:
    """解析鱼竿定义。
...
def _effective_luck(player: dict[str, Any], cfg: dict[str, Any] | None = None) -> float:
    ...
    * ``buff_quality``：持续型 buff（锦鲤玉佩），只在 ``buff_casts_left > 0`` 时生效，
    ...
    ``_roll_quality_mult`` 里与鱼饵/鱼竿/天气的手气**一起**钳到 ``luck_cap``
    （v1.18.22 起可配，默认 2.0；老版本硬编码 1.0，后期鱼饵+鱼竿就顶满，
    玉佩/玉髓灯全成废纸），所以叠起来不会把品质分布顶穿。
...
def _quality_ceil() -> float:
    ...
    站长可以往 ``quality_tiers`` 里加更高的档（v1.18.22 起神品默认就是 6.0-10.0），

...
v1.10.0 的四项需求（玩家金币编辑 / 命令别名 / 自定义命令 / 配置面板瘦身）与
v1.11.0 的**养成道具重做**（三种角色：喂鱼 / 钓手手气 / 鱼缸装饰）都已交付，
详见 README 的 `### v1.11.0` 与 `### v1.10.0` 更新日志。下一轮想做什么，先和用户确认再往这里写。

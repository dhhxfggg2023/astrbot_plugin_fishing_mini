...
**还没修的同款漏洞（待站长点头）**：挂机收益 `/钓鱼 水族馆 领` 仍是
`当前馆藏估值 × 距上次领取小时数`，所以「先把缸清空攒满 12 小时、领之前把好鱼塞进去」
就能按满额结算（鱼只需要在缸里待一瞬）。修法现成：同一个 `tank_since`/`tank_seconds`，
把 `total_value` 换成「每条鱼按各自在缸时长加权」。改动会影响玩家实际收益，
所以先问再动（B 方案）。

...
async def main():
    ...
    # ⚠️ v1.18.75：站长抽到「深渊秘饵」后以为没拿到（背包里只有凭证、看不到「已经在生效」）。
    # 背包的限用道具那一栏要把「凭证 -> 用什么特权」写清，别让他以为白抽了。
    check(
        "深渊秘饵" in _bag_lb and "全图鱼口" in _bag_lb,
        f"限用凭证写明它对应的特权 -> "
        f"{[l for l in _bag_lb.splitlines() if '秘饵' in l][:1]}",
    )
    # 抛一竿：凭证 -1、这一竿按秘饵的特权走
    ...
    # =====================================================================
    print("\n[19] 回复里的称呼（昵称 / @）：第一条回复才加、按钮路径也加")
    # 旧写法把 @ 包在最外层，而 QQ 官方「带按钮的回复」是直发的（_send_with_buttons
    # 发完就 return，走不到 yield 那条链）—— 于是被整个跳过。现在改成在统一出口
    # `_say` 里加，两条路径都覆盖。这一节就是钉住这个「按钮路径也被称呼到」。

    class _MentionEvent(FakeEvent):
        def __init__(self, sid, mid="M1", name="测试玩家"):
            super().__init__(sid, sender_name=name)
            self.message_obj = type("_M", (), {"message_id": mid})()

    _mn = make_plugin()
    check(_mn._mention_prefix(_MentionEvent("98001", "M1")) == "测试玩家\n", "默认「昵称」档：开头写昵称")
    check(_mn._mention_prefix(_MentionEvent("98001", "M1")) == "", "同一条消息只称呼一次（后面的回复不再加）")
    check(_mn._mention_prefix(_MentionEvent("98001", "M2")) == "测试玩家\n", "换一条消息又会称呼")

    _mn_off = make_plugin(dict(_CFG, mention_mode="关闭"))
    check(_mn_off._mention_prefix(_MentionEvent("98002", "O1")) == "", "「关闭」档不称呼")

    _mn_at = make_plugin(dict(_CFG, mention_mode="@"))
    check(_mn_at._mention_prefix(_MentionEvent("98003", "A1")) == "@测试玩家\n", "「@」档写成 @昵称（纯文本）")

    # 拿不到 message_id / 昵称时都不称呼，也**不占**这一次的名额
    check(_mn._mention_prefix(_MentionEvent("98004", "")) == "", "拿不到 message_id 就不称呼")
    check(_mn._mention_prefix(_MentionEvent("98005", "N1", name="")) == "", "拿不到昵称就不称呼")

    # 按钮路径：_say 会先把称呼并进文本，再交给 _send_with_buttons
    _cap_btn: list[str] = []
    _mn_btn = make_plugin()
    ...
    _mn_btn._send_with_buttons = _fake_send_ok
    await cmd(_mn_btn, _MentionEvent("98006", "BTN-1"), "帮助", "1")
    check(
        bool(_cap_btn) and _cap_btn[0].startswith("测试玩家\n"),
        f"按钮那条路径也带称呼 -> {(_cap_btn[0] if _cap_btn else '（没发出去）')[:14]!r}",
    )
    ...
    _mt_out = text_of(await cmd(_mn_txt, _MentionEvent("98007", "PL-1"), "帮助", "1"))
    check(
        _mt_out.startswith("测试玩家\n"),
        f"纯文本路径也带称呼 -> {_mt_out.splitlines()[0] if _mt_out else '（空）'}",
    )
    make_plugin()  # 恢复默认（模块级共享表）
